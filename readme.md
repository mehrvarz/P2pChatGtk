...
P2pChatGtk builds upon P2pChatSMP and P2pCore. The `lib` folder contains jar files build from these projects source code.
...
Two secret words must be entered to start a secure chat session. This means the two clients must agree upon the two secrets before the chat session begins. The first secret string will be used to match and connect the two clients. The second secret string will be used as the OTR/SMP secret to create encryption keys dynamically. Both clients must use the exact same secret secrets. As soon as a P2P connection has been established, OTR and SMP will be started automatically. A few seconds later, the SMP handshake should be completed and a secure and private conversation can take place. 
...
`makegtkjar` script will run proguard in order to combine all required archives and will generate a single, runnable `P2pChatGtk.jar`.
